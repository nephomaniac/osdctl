...
ACTION="$1"
REASON="${TEST_REASON:-ROSAENG-2057}-mutating"
    ...
    oao-state)
        ...
        ocm backplane elevate "$REASON" -- get secrets -n openshift-ocm-agent-operator -o json 2>/dev/null | \
            ...
            while IFS=$'\t' read -r name b64token; do
                hash=$(echo "$b64token" | base64 -d 2>/dev/null | $SHA256CMD | cut -c1-12)
    ...
    wait-for-oao-reconcile)
        ...
        while [ "$ELAPSED" -lt "$TIMEOUT" ]; do
            ...
            if [ -n "$CURRENT_RV" ] && [ "$CURRENT_RV" != "$PRE_RV" ]; then
                ...
                TOKEN_HASH=$(ocm backplane elevate "$REASON" -- get secret ocm-access-token -n openshift-ocm-agent-operator -o jsonpath='{.data.access_token}' 2>/dev/null | base64 -d 2>/dev/null | $SHA256CMD | cut -c1-12)
