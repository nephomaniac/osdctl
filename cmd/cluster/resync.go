...
`,
	...
	resyncCmd.Flags().StringVar(&r.hiveOcmUrl, "hive-ocm-url", "", "(optional) OCM environment URL for hive operations. Valid values: 'production', 'staging', 'integration'. If not specified, uses the same OCM environment as the target cluster.")
...
func (r *Resync) initWithMultiEnv(scheme *runtime.Scheme) error {
	...
	if err != nil {
		return fmt.Errorf("failed to create hive k8s client: %w", err)
	}
