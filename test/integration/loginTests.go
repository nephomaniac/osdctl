...
OCM and kube client connections, and perform basic requests for each connection in
...
// newCmdLoginTests defines a command to run through a series of tests to validate existing and new and legacy
// ocm + backplane client functions
func newCmdLoginTests() *cobra.Command {
	ops := newTestHiveLoginOptions()
	testHiveLoginCmd := &cobra.Command{
		Use:               "login-tests",
		Short:             "Test utility to exercise OSDCTL client connections for both Target Cluster and its Hive Cluster.",
...
// This function is primarily used as a sample operation for POC client testing.
...
func getClusterDeployment(hiveKubeClient client.Client, clusterID string) (cd *hivev1.ClusterDeployment, err error) {
	...
	if err := hiveKubeClient.List(context.TODO(), &cds, &client.ListOptions{}); err != nil {
		fmt.Printf("Error fetching cluster deployments, err:'%v'", err)
		return nil, err
	}
...
func testK8sNewAsBackplaneClusterAdmin(hiveCluster *v1.Cluster, hiveOCM *sdk.Connection, clusterID, reason string) error {
	...
	fmt.Println("Create and test Kube Client with k8s.NewAsBackplaneClusterAdminWithConn() - PASS")
	printDiv()
	return nil
}
